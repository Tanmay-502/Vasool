...
import { TERMINAL_STATUSES } from "@/lib/recovery-actions.mjs";
          ...
          <main className="mx-auto max-w-[1480px] px-4 py-6 sm:px-6 xl:px-10 xl:py-8">
            ...
            <section id="audit-ledger" className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1.5fr)_340px]">
              ...
              <aside className="space-y-5">
                ...
                <div className="rounded-3xl border border-[#dfe5ef] bg-[#0d1426] p-5 text-white shadow-sm"><div className="text-[10px] font-data uppercase tracking-[0.16em] text-white/40">Audit ledger</div><h3 className="font-display mt-1 text-base font-bold">Recent decision events</h3><div className="mt-4 space-y-4">{audit.length ? audit.slice(0, 7).map((entry) => <AuditItem key={entry.id} entry={entry} />) : <div className="rounded-xl border border-white/10 bg-white/5 p-4 text-xs text-white/50">No recent audit events.</div>}</div></div>
              </aside>
            </section>
          </main>
        </div>
...
function CaseDetailView({ detail, busyAction, killSwitch, onAnalyze, onReview, onExecute }: { detail: CaseDetail; busyAction: string; killSwitch: boolean; onAnalyze: () => void; onReview: (decision: "approve" | "reject") => void; onExecute: () => void }) {
  ...
  const meta = statusMeta[item.status] ?? { label: item.status.replaceAll("_", " "), tone: "neutral" as const };
  const terminalCase = TERMINAL_STATUSES.has(item.status);
...
function Context({ value, label }: { value: string; label: string }) { return <div className="rounded-xl bg-[#f7f9fc] p-3"><div className="text-[10px] uppercase tracking-wide text-[#8a93a5]">{label}</div><div className="mt-1 truncate text-xs font-bold text-[#394257]">{value}</div></div>; }

function formatTime(value: string) { try { return new Intl.DateTimeFormat("en-IN", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }).format(new Date(value)); } catch { return value; } }
